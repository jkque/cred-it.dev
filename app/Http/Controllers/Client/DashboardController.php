    ...
    public function simulatorAdd(Request $request){
        ...
        if($day >= 6 && $day <= 20){
            ...
            if($due && count($due) > 0){
                ...
                $new_data = ['user_id'=>$user_id, 'amount'=>$due_amount, 'last_modified'=>date('Y-m-d H:i:s')];
                DB::table('due')->whereRaw("UNIX_TIMESTAMP(due_date) = UNIX_TIMESTAMP('".$due_date."')")->update($new_data);
            }else{
                $new_data = ['user_id'=>$user_id, 'amount'=>$data['amount'], 'last_modified'=>date('Y-m-d H:i:s'), 'due_date'=>$due_date];
